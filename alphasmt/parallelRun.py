...
logic_strategy = {
    "QF_SLIA": "/home/paul/Dokumente/z3alpha/test.txt"
}
...
def run_z3_with_command(smt_file, command, logic):
    ...
    if logic in ["QF_S", "QF_SLIA", "QF_SNIA"]:
        result = subprocess.run(['z3', "z3str", temp_file], capture_output=True, text=True)
    else:
...
def main(smt_file):
    ...
    with ThreadPoolExecutor(max_workers=4) as executor:
        futures = [executor.submit(run_z3_with_command, smt_file, command, logic) for command in commands]
