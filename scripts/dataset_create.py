...
def main():
    parser = argparse.ArgumentParser(
        description="Create train/test split dataset from files in a folder."
    )
    parser.add_argument(
        "--train_size",
        type=int,
        required=True,
        help="Number of benchmarks to put in train/; the rest go into test/",
    )
    parser.add_argument(
    ...
    log.info("%s", "=" * 60)
    log.info("Train size: %s", args.train_size)
    ...
    assert train_size <= all_file_size, (
        f"--train_size {train_size} exceeds total file count {all_file_size}"
    )
    test_size = all_file_size - train_size
    ...
    create_dir(test_dir, all_files[train_size:])
...
# sample usage: python dataset_create.py --train_size 750 --benchmark_dir smtlib/QF_NIA/all --dataset_dir smtlib/QF_NIA
# sample usage with CSV: python dataset_create.py --train_size 750 --dataset_dir dataset --csv_file data/hard/qfnia_2unsolve.csv --prefix smtlib
